...
-- question 1 On veut obtenir la liste des codes candidats et des notes obtenues à l'épreuve d'économie (code épreuve = 6). 
...
--question 2 On souhaiterait afficher le code élève, le code épreuve, la note, 
--le coefficient et la note coefficientée (note * coeff).

select Numcandidat, Epreuve.codeEpreuve , Note , coeff , Note*coeff as NoteCoefficientée
...
select codeepreuve, SUM(note) as TotalDesNotes
from notation 
group by CodeEpreuve

--question 4 On désire connaître le nombre de candidats inscrits par établissement.

Select codeetab , count(Numcandidat) as effectif_par_lyccée
from candidat 
group by codeetab;

--question 5 On voudrait obtenir les statistiques suivantes (regroupées par code épreuve).
--On souhaite obtenir le résultat suivant :

select codeepreuve , avg(note) as MoyenneDeNote , min(note) as MinimumDeNote , max(note) as MaxDeNote
from notation 
group by codeepreuve;
...
select Numcandidat , sum(note) / count(note) as moyenne
from notation 
where codeepreuve='6'
group by Numcandidat
having sum(note) / count(note) >=10  --on peut pas utiliser 'moy' qui est juste un surnommage !!!
order by 2 DESC;  --2 signifie : colonne ou champs 2 (la moyenne)
