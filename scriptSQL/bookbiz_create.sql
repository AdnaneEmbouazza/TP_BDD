CREATE TABLE and DROP TABLE commands for Informix
...
create table titleauthors
	...
	constraint fk_titleauthors_title_id foreign key(title_id) references titles(title_id)
	);
...
create table salesdetails
	...
	date_shipped date ,
	constraint pk_salesdetails primary key(sonum,title_id),
	constraint fk_salesdetails_sonum foreign key(sonum) references sales(sonum),
	constraint fk_salesdetails_title_id foreign key(title_id) references titles(title_id)
	 );
...
create table titleditors
 ...
 title_id char(6) not null,
 ed_ord int ,
 constraint pk_titleditors primary key(ed_id,title_id),
 constraint fk_titleditors_ed_id foreign key(ed_id) references editors(ed_id),
constraint fk_titleditors_title_id foreign key(title_id) references titles(title_id)
 );
...
--3 
select ed_lname, ed_fname from editors 
where state='CA';

--4 
select concat(ed_lname,ed_fname)  as nom_prenom from editors
where state='CA';

--5
select concat('le nom du publieur' , ed_id ,'est' ed_lname) as phrase_auteurs
from editors;
